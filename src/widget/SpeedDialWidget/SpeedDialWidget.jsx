...
const SpeedDialWidget = () => {
    ...
    return (
        <SpeedDial
            ariaLabel="SpeedDial"
            sx={{ position: 'fixed', bottom: responsive ? 30 : 20, right: responsive? 45 : 5, color: 'rgba(45, 152, 218, 1)' }}
            icon={<FontAwesomeIcon
                color='rgba(255, 255, 255, 1)' icon={faPhone} style={{ height: '18px', width: '18px' }} />}
        >
            {actions.map((action) => (
                <SpeedDialAction
                    key={action.name}
